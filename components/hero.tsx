...
import { brandName, heroMetrics } from "@/lib/portal-data";
...
export function Hero({ startColors, endColors }: HeroProps = {}) {
  ...
  return (
    <section id="hero" className="hero relative min-h-screen w-full bg-background overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-360 flex-col px-6 pt-40 pb-10 text-left sm:px-12 sm:pt-44 sm:pb-12 lg:px-24 2xl:max-w-450 3xl:max-w-550" style={{ perspective: "1200px" }}>
        <div className="flex flex-1 flex-col justify-center">
        <div className="mb-5 inline-flex w-fit items-center gap-2 rounded-full border border-foreground/10 bg-background/35 px-4 py-2 text-xs uppercase tracking-[0.24em] text-foreground/55 backdrop-blur-lg">
          <span className="h-2 w-2 rounded-full bg-emerald-400" />
          MainNet live operational workspace
        </div>
        ...
        <h1 className="text-[clamp(3rem,8vw,12rem)] leading-[1.05] tracking-tight text-foreground">
          ...
          <span className="block overflow-hidden pb-[0.1em]">
            <motion.span
              ...
              style={{ transformOrigin: "center bottom", transformStyle: "preserve-3d" }}
            >
              <em className="font-serif">validators.</em>
            </motion.span>
          </span>
        </h1>

        <motion.p
          className="mt-8 max-w-2xl text-[clamp(1.125rem,1.5vw,1.75rem)] leading-relaxed text-foreground/80"
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ duration: 1, ease: [0.25, 1, 0.5, 1], delay: 1.2 }}
        >
          {brandName} is the enterprise-grade operating layer for node health, rewards analytics,
          Foundation communication, upgrade readiness, and compliance visibility across Canton environments.
        </motion.p>
        ...
        >
          ...
          <a
            href="/leaderboard"
            className="inline-flex items-center justify-center rounded-full border border-foreground/15 bg-background/40 px-6 py-3 text-base font-medium text-foreground backdrop-blur-lg transition-opacity hover:opacity-80"
          >
            View Leaderboard
          </a>
        </motion.div>
      ...
      >
        <div className="grid gap-3 rounded-3xl bg-background/40 p-4 backdrop-blur-lg sm:grid-cols-2 xl:grid-cols-6">
          {heroMetrics.map((metric) => (
            <div key={metric.label} className="rounded-2xl border border-foreground/8 bg-background/35 p-4">
              <p className="text-[11px] uppercase tracking-[0.2em] text-foreground/50">{metric.label}</p>
              <p className="mt-2 text-2xl font-medium tracking-tight text-foreground/90">{metric.value}</p>
              <p className="mt-2 text-sm leading-6 text-foreground/55">{metric.note}</p>
            </div>
          ))}
        </div>
      </motion.div>
